...
This class implements a min-heap data structure, where the smallest element is always at the root. It supports common heap operations such as insertion, deletion, and retrieval.

Methods:
    - add(e): Adds a new element to the heap while maintaining the heap property.
    - remove(): Removes and returns the smallest element (root) from the heap.
    - getSize(): Returns the number of elements in the heap.
    - isEmpty(): Checks if the heap is empty.
    - peek(): Retrieves the smallest element without removing it.
    - getLst(): Returns the underlying list representing the heap.
...
Usage Example:
    ...
    >>> print(heap.getLst())  # Output: [10, 15]
"""
